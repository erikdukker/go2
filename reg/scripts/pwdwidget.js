...
function pwWidget(divid,pwdname)
{
	...
	this.showing_pwd=1;
	this.txtShow = 'Laat zien';
	this.txtMask = 'Verberg';
...
function _MakePWDWidget()
{
	...
	this._showpwStrength = pwStrength;
...
function generatePWD()
{
	...
	for(i=0;i<3;i++)
	{
		pw += String.fromCharCode("a".charCodeAt(0) + getRand(maxAlpha));
	}
	for(i=0;i<3;i++)
	{
		pw += String.fromCharCode("A".charCodeAt(0) + getRand(maxAlpha));
	...
	for(i=0;i<3;i++)
	{
		pw += String.fromCharCode("0".charCodeAt(0) + getRand(10));
	}
	for(i=0;i<4;i++)
	{
		pw += strSymbols.charAt(getRand(strSymbols.length));
	}
