...
if (isset($_SESSION['lg'])){
	...
	echo "<input id='s|".$somNo."' 	name='s|".$somNo."' type='text' style='font-size:12px' size='200' value = '".$somTx."' >".PHP_EOL;	
} else {
	echo "<input id='k|".$ken."' 	name='k|".$somNo."' type='hidden' value = '".$ken."' />";	
	echo "<input id='p|".$somNo."' 	name='p|".$somNo."' type='hidden' value = '".$pasTx."' />";	
	echo "<input id='s|".$somNo."' 	name='s|".$somNo."' type='hidden' value = '".$somTx."' />".PHP_EOL;
}
...
if ($vr == 'mk') {
	...
	$ag=1;
	for ( $i= 1; $i <= $pas['aa']; $i++) {if (isset($som['a'.$i]) and $som['a'.$i] == $som['rs']) {$ok = $i;}}
